...
def test_total_actions_calculation():
    ...
    expected = (ActionSpace.MOVE_DIMENSION * 
...
def test_interpret_action_negative():
    ...
    with pytest.raises(ValueError, match="You cannot give a negative action to interpret_action"):
...
def test_with_modified_dimensions():
    ...
    try:
        ...
        with pytest.raises(ValueError):
            ActionSpace.encode_action([3, 0, 0, 0])  # Invalid move direction for new dimensions
...
def test_action_space_consistency():
    ...
    original_dims = (ActionSpace.MOVE_DIMENSION, ActionSpace.ATTACK_DIMENSION,
                    ActionSpace.PHASE_DIMENSION, ActionSpace.BOMB_DIMENSION)
    ...
    try:
        ...
        total_actions = (ActionSpace.MOVE_DIMENSION * ActionSpace.ATTACK_DIMENSION *
                        ActionSpace.PHASE_DIMENSION * ActionSpace.BOMB_DIMENSION)
        ...
        # Restore original dimensions
        (ActionSpace.MOVE_DIMENSION, ActionSpace.ATTACK_DIMENSION,
         ActionSpace.PHASE_DIMENSION, ActionSpace.BOMB_DIMENSION) = original_dims