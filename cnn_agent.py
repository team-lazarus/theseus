...
class DualActionAgent:
    ...
    def select_actions(self, state):
        ...
        if self.movement_agent.training and random.random() < self.movement_agent.epsilon:
            movement_action = random.randint(0, self.movement_agent.policy_network.layer3.out_features - 1)
        else:
            with torch.no_grad():
        ...
        if self.attack_agent.training and random.random() < self.attack_agent.epsilon:
            attack_action = random.randint(0, self.attack_agent.policy_network.layer3.out_features - 1)
        else:
            with torch.no_grad():
    ...
    def train_episode(self, environment):
        ...
        while not terminated:
            ...
            self.movement_agent.memory.append((state, movement_action, next_state, reward, terminated))
            self.attack_agent.memory.append((state, attack_action, next_state, reward, terminated))
    ...
    def learn(self):
        ...
        if len(self.movement_agent.memory) > self.movement_agent.mini_batch_size:
            mini_batch = self.movement_agent.memory.sample(self.movement_agent.mini_batch_size)
        ...
        if len(self.attack_agent.memory) > self.attack_agent.mini_batch_size:
            mini_batch = self.attack_agent.memory.sample(self.attack_agent.mini_batch_size)
    ...
    def load_models(self, movement_path, attack_path):
        ...
        self.movement_agent.target_network.load_state_dict(self.movement_agent.policy_network.state_dict())
        
        self.attack_agent.policy_network.load_state_dict(torch.load(attack_path))
        self.attack_agent.target_network.load_state_dict(self.attack_agent.policy_network.state_dict())
